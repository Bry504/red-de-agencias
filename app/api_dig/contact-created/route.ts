...
function parseDateToISO(raw: string | null): string | null {
  ...
  console.warn('[DIG contact-created] Fecha no válida, se envió:', raw);
  return null;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[DIG contact-created] token query =', tokenFromQuery);

    if (!WEBHOOK_TOKEN || !tokenFromQuery || tokenFromQuery !== WEBHOOK_TOKEN) {
      console.warn(
        '[DIG contact-created] Token inválido:',
        tokenFromQuery,
        'esperado:',
    ...
    if (!isRecord(rawBody)) {
      console.error('[DIG contact-created] Body no es objeto:', rawBody);
      return NextResponse.json(
    ...
    console.log(
      '[DIG contact-created] root =',
      JSON.stringify(root, null, 2)
    );
    console.log(
      '[DIG contact-created] contact =',
      JSON.stringify(contact, null, 2)
    );
    console.log(
      '[DIG contact-created] customData =',
    ...
    console.log('[DIG contact-created] Campos mapeados:', {
      nombre_completo,
      celular,
      email,
      nombre_anuncio,
    ...
    if (!celular && !hl_contact_id) {
      console.warn(
        '[DIG contact-created] Sin celular ni hl_contact_id. No se inserta.'
      );
      return NextResponse.json(
        {
          ok: false,
    ...
    if (error) {
      console.error('[DIG contact-created] Supabase insert error:', error);
      return NextResponse.json(
    ...
    console.log('[DIG contact-created] Insert OK, id =', data.id);

    return NextResponse.json({ ok: true, id: data.id }, { status: 201 });
  } catch (err: unknown) {
    console.error('[DIG contact-created] Error inesperado:', err);
    const message =
