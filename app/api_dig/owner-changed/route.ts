...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[DIG owner-changed] token query =', tokenFromQuery);

    if (!WEBHOOK_TOKEN || !tokenFromQuery || tokenFromQuery !== WEBHOOK_TOKEN) {
      console.warn(
        '[DIG owner-changed] Token inválido:',
        tokenFromQuery,
        'esperado:',
    ...
    if (!isRecord(rawBody)) {
      console.error('[DIG owner-changed] Body no es objeto:', rawBody);
      return NextResponse.json(
    ...
    console.log('[DIG owner-changed] root =', JSON.stringify(root, null, 2));
    console.log(
      '[DIG owner-changed] customData =',
    ...
    console.log('[DIG owner-changed] Campos limpios:', cleaned);

    if (!cleaned.hl_opportunity_id) {
      console.warn('[DIG owner-changed] Falta hl_opportunity_id (oportunidad)');
      return NextResponse.json(
    ...
    if (!cleaned.propietario_ghl_id) {
      console.warn('[DIG owner-changed] Falta propietario (ghl_id)');
      return NextResponse.json(
    ...
    if (oppError) {
      console.error(
        '[DIG owner-changed] Error buscando oportunidad por hl_opportunity_id:',
        oppError
      );
      return NextResponse.json(
    ...
    if (!oppRow) {
      console.warn(
        '[DIG owner-changed] No se encontró oportunidad con hl_opportunity_id =',
    ...
    // 🚫 NUEVA REGLA: si la oportunidad NO tiene propietario actual -> skip
    if (!propietarioAnteriorId) {
      console.warn(
        '[DIG owner-changed] Oportunidad sin propietario actual (propietario_id NULL). Se omite registro de reasignación.'
      );
      return NextResponse.json(
        {
          ok: true,
          skipped: true,
          reason: 'empty_previous_owner',
        },
        { status: 200 }
      );
    }
    ...
    if (nuevoUsuarioError) {
      console.error(
        '[DIG owner-changed] Error buscando nuevo propietario en usuarios:',
        nuevoUsuarioError
      );
      return NextResponse.json(
    ...
    if (!nuevoUsuarioRow?.id) {
      console.warn(
        '[DIG owner-changed] No se encontró usuario con ghl_id =',
    ...
    if (propietarioAnteriorId === propietarioActualId) {
      console.log(
        '[DIG owner-changed] Mismo propietario. No se registra reasignación.'
      );
      return NextResponse.json(
    ...
    console.log(
      '[DIG owner-changed] insertPayload reasignaciones =',
    ...
    if (insertError) {
      console.error(
        '[DIG owner-changed] Error insertando en reasignaciones:',
        insertError
      );
      return NextResponse.json(
    ...
    console.log(
      '[DIG owner-changed] Reasignación registrada, id =',
      inserted?.id ?? null
    );
    ...
    console.error('[DIG owner-changed] Error inesperado:', err);
    return NextResponse.json(
