...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[DIG note-created] token query =', tokenFromQuery);

    if (!WEBHOOK_TOKEN || !tokenFromQuery || tokenFromQuery !== WEBHOOK_TOKEN) {
      console.warn(
        '[DIG note-created] Token inválido:',
        tokenFromQuery,
        'esperado:',
    ...
    if (!isRecord(rawBody)) {
      console.error('[DIG note-created] Body no es objeto:', rawBody);
      return NextResponse.json(
    ...
    console.log('[DIG note-created] root =', JSON.stringify(root, null, 2));
    console.log(
      '[DIG note-created] customData =',
    ...
    console.log('[DIG note-created] Campos limpios:', cleaned);
    ...
    if (cleaned.propietario_ghl_id) {
      ...
      if (usuarioError) {
        console.error(
          '[DIG note-created] Error buscando propietario en usuarios:',
          usuarioError
        );
        ...
        console.warn(
          '[DIG note-created] SKIP propietario: no se encontró usuario con ghl_id =',
          cleaned.propietario_ghl_id,
          '→ se insertará nota con propietario = null'
        );
      }
    } else {
      console.warn(
        '[DIG note-created] Payload sin propietario (ghl_id). Se insertará nota con propietario = null.'
      );
    }
    ...
    if (cleaned.contacto_hl_id) {
      ...
      if (contactoError) {
        console.error(
          '[DIG note-created] Error buscando contacto en contactos:',
          contactoError
        );
        ...
        console.warn(
          '[DIG note-created] SKIP contacto: no se encontró contacto con hl_contact_id =',
          cleaned.contacto_hl_id,
          '→ se insertará nota con contacto = null'
        );
      }
    } else {
      console.warn(
        '[DIG note-created] Payload sin contacto (hl_contact_id). Se insertará nota con contacto = null.'
      );
    }
    ...
    console.log(
      '[DIG note-created] insertPayload notas =',
    ...
    if (insertError) {
      console.error(
        '[DIG note-created] Error insertando nota:',
        insertError
      );
      return NextResponse.json(
    ...
    console.log(
      '[DIG note-created] Insert OK en notas, id =',
      inserted?.id ?? null
    );
    ...
    console.error('[DIG note-created] Error inesperado:', err);
    return NextResponse.json(
