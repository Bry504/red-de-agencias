...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[TRAD appointment] token query =', tokenFromQuery);

    if (!WEBHOOK_TOKEN || !tokenFromQuery || tokenFromQuery !== WEBHOOK_TOKEN) {
      console.warn(
        '[TRAD appointment] Token inválido:',
        tokenFromQuery,
        'esperado:',
    ...
    if (!isRecord(rawBody)) {
      console.error('[TRAD appointment] Body no es objeto:', rawBody);
      return NextResponse.json(
    ...
    console.log('[TRAD appointment] root =', JSON.stringify(root, null, 2));
    console.log(
      '[TRAD appointment] customData =',
    ...
    if (cleaned.contacto_raw) {
      ...
      console.warn(
        '[TRAD appointment] Payload sin contacto. Se insertará oportunidad = null.'
      );
    }
    ...
    if (cleaned.propietario_ghl_id) {
      ...
      if (usuarioError) {
        console.error(
          '[TRAD appointment] Error buscando propietario en usuarios:',
          usuarioError
        );
        ...
        console.warn(
          '[TRAD appointment] No se encontró usuario con ghl_id =',
          cleaned.propietario_ghl_id
        );
      }
    } else {
      console.warn(
        '[TRAD appointment] Payload sin propietario (ghl_id). Se insertará propietario = null.'
      );
    }
    ...
    console.log(
      '[TRAD appointment] insertPayload citas_programadas =',
    ...
    if (insertError) {
      console.error(
        '[TRAD appointment] Error insertando cita_programada:',
        insertError
      );
      return NextResponse.json(
    ...
    console.log(
      '[TRAD appointment] Insert OK en citas_programadas, id =',
      inserted?.id ?? null
    );
    ...
    console.error('[TRAD appointment] Error inesperado:', err);
    return NextResponse.json(
