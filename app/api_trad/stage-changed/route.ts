...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[TRAD stage-changed] token query =', tokenFromQuery);

    if (!WEBHOOK_TOKEN || !tokenFromQuery || tokenFromQuery !== WEBHOOK_TOKEN) {
      console.warn(
        '[TRAD stage-changed] Token inválido:',
        tokenFromQuery,
        'esperado:',
    ...
    if (!isRecord(rawBody)) {
      console.error('[TRAD stage-changed] Body no es objeto:', rawBody);
      return NextResponse.json(
    ...
    console.log(
      '[TRAD stage-changed] root =',
      JSON.stringify(root, null, 2)
    );
    console.log(
      '[TRAD stage-changed] opportunity =',
      JSON.stringify(opportunityObj, null, 2)
    );
    console.log(
      '[TRAD stage-changed] customData =',
    ...
    if (!hlOpportunityId) {
      console.error(
        '[TRAD stage-changed] Body sin hl_opportunity_id reconocible:',
        root
      );
      return NextResponse.json(
    ...
    console.log('[TRAD stage-changed] Campos limpios:', cleaned);
    ...
    if (!etapaDestino) {
      console.warn(
        '[TRAD stage-changed] Sin etapa_destino. No se inserta historial.'
      );
      return NextResponse.json(
    ...
    if (!etapaOrigen && etapaDestino === 'Oportunidad recibida') {
      console.log(
        '[TRAD stage-changed] Cambio inicial (Oportunidad recibida sin etapa_origen). Se omite porque lo crea Supabase.'
      );
      return NextResponse.json({
        ok: true,
        skipped: true,
    ...
    if (oppError) {
      console.error(
        '[TRAD stage-changed] Error buscando oportunidad por hl_opportunity_id:',
        oppError
      );
      return NextResponse.json(
    ...
    if (!oppRow) {
      console.warn(
        '[TRAD stage-changed] No se encontró oportunidad con hl_opportunity_id =',
    ...
    //    (misma lógica que tu código anterior, pero por oportunidad)
    // --------------------------------------------------
    if (!etapaOrigen) {
      ...
      if (lastStageError) {
        console.error(
          '[TRAD stage-changed] Error obteniendo última etapa previa:',
          lastStageError
        );
      } else if (lastStage && lastStage.etapa_destino) {
        etapaOrigen = lastStage.etapa_destino;
      }
    }
    ...
    if (cleaned.propietario_ghl_id) {
      ...
      if (usuarioError) {
        console.error(
          '[TRAD stage-changed] Error buscando propietario en usuarios:',
          usuarioError
        );
        ...
        console.warn(
          '[TRAD stage-changed] No se encontró usuario con ghl_id =',
          cleaned.propietario_ghl_id
        );
      }
    } else {
      console.warn(
        '[TRAD stage-changed] Payload sin propietario (ghl_id). Se insertará propietario = null.'
      );
    }

    // --------------------------------------------------
    // 9) Insertar en historial_etapas (columnas con el mismo nombre)
    // --------------------------------------------------
    ...
    console.log(
      '[TRAD stage-changed] insertPayload historial_etapas =',
    ...
    if (insertError) {
      console.error(
        '[TRAD stage-changed] Error insertando historial_etapas:',
        insertError
      );
      return NextResponse.json(
    ...
    console.log(
      '[TRAD stage-changed] Insert OK en historial_etapas, id =',
      inserted?.id ?? null
    );
