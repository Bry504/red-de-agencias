...
// Celular Perú: exige al menos 9 dígitos, devuelve +51XXXXXXXXX o null
function normalizarCelularPeru(raw: string | null | undefined): string | null {
  if (!raw) return null;
  const digits = raw.replace(/\D/g, '');

  // Si no llega a 9 dígitos, lo consideramos incompleto / inválido
...
// Generar email sintético a partir del nombre:
// nombre_normalizado.autogenerado@gmail.com
function buildAutogeneratedEmailFromName(nombre: string): string {
  // Normalizar: quitar tildes, pasar a minúsculas y limpiar caracteres raros
  let base = nombre
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '') // quitar tildes
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '.') // reemplazar todo lo que no sea alfanumérico por puntos
  ...
  return `${base}.autogenerado@gmail.com`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Si NO hay celular válido, generamos un email sintético basado en el nombre
    let email: string | null = null;
    if (!phone) {
      email = buildAutogeneratedEmailFromName(nombre_completo);
    }
    ...
    if (notaTexto) {
      try {
        ...
        if (!noteRes.ok) {
          ...
          console.error(
            'GHL note error (entorno-personal):',
            noteRes.status,
            text
          );
        }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              'El nombre del contacto ya ha sido registrado previamente, por favor agréguele un caracter diferente.',
            detail:
